...
{ return clubs == NULL || clubs->clubs_head == NULL; }
...
void cls_delete_node(sClubs clubs, sClubNode * ptr_club_to_delete)
{
...
sClubNode findClub(sClubs clubs, const char * string, bool (*find)(const sClubNode, const char *))
{
    sClubNode current = cls_get_tail(clubs);
    while( current && !find(current, string) )
        current = current->club_prev;
    return current;
}

bool ifClubCountry(const sClubNode club, const char * country)
{
    return !strcmp(club->club_country, country);
}
