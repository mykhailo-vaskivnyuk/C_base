...
sClubNode * cls_exng(sClubs * clubs, sClubNode * club_current)
{
    //club_current != NULL
    //це означає, що у списку мінімум один елемент
    //припускаємо, що club_current завжди належить списку clubs
    sClubNode * club_prev = NULL;
    sClubNode * club_next = NULL;

    if( !(club_next = club_current->club_next) ) return NULL;
    if(   club_current != clubs->clubs_head    ) club_prev = clubs->clubs_head;
/*
    while( club_prev->club_next && club_prev->club_next != club_current )
                club_prev = club_prev->club_next;

    club_next = club_current->club_next;

    club_prev->club_next = club_next;
    club_current->club_next = club_current->club_next->club_next;
    club_next->club_next = club_current;
*/
    return club_next;
}
