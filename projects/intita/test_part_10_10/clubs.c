...
{
    if( (*ptr_clubs)->clubs_head == *ptr_club_to_delete )
    {
        if( (*ptr_club_to_delete)->club_next == NULL)
        {
            cls_delete_list(ptr_clubs);
            return;
        }

        (*ptr_clubs)->clubs_head = (*ptr_club_to_delete)->club_next;
        (*ptr_clubs)->clubs_count--;
        cls_free_node(ptr_club_to_delete);
        return;
    }

    sClubNode * club_current = (*ptr_clubs)->clubs_head;
    while( club_current != NULL && club_current->club_next != *ptr_club_to_delete )
        club_current = club_current->club_next;
    if( club_current == NULL)
        return;

    club_current->club_next = (*ptr_club_to_delete)->club_next;
    cls_free_node(ptr_club_to_delete);
    (*ptr_clubs)->clubs_count--;
}
