 ...
 * Частина 5. Динамічне виділення пам’яті: функція realloc().
 *----------------------------------------------------------------
 * ITA17
 * Vaskivnyuk Mykhailo
 * 13.06.2017
...
/* ЗАВДАННЯ
Протестуй функцію realloc(), реалізувавши наступні маніпуляції:
...
3) в циклі заповнити масив стуркутр даними про клуби (використовуючи покажчик на виділену в п. 2 область пам'яті).
4) для контролю - вивести дані про клуби на екран (за допомогою того самого покажчика);
5) оголосити новий аналогічний за розміром масив структур і скопіювати в нього існуючий;
6) виділити для раніше введеного покажчика на масив структур область пам’яті, меншу на розмір 2 структур;
7) в нову область пам’яті записати по черзі структури з існуючого масиву в порядку зростання поля place (окрім двох останніх);
8) новий масив вивести на екран і переконатися в коректності проведеного “сортування” клубів.
*/
...
int  copyClubs     (sClub_ptr,   int,    sClub_ptr, int,    int);
void sortClubs     (sClub_ptr,   int);
void cutClubs      (sClub_ptr *, int *,  int);
...
//-----------------------------------------------------------------------------
//  create second array, copy data from first to second
//-----------------------------------------------------------------------------

    for(i = 0; i < clubs_count; i++)
        addClub(&clubs_2, &clubs_count_2, "", "", 0, 0);

    copyClubs(clubs_2, clubs_count_2, clubs, clubs_count, 0);
...
//-----------------------------------------------------------------------------
//  cut second, sort first, copy first to second
//-----------------------------------------------------------------------------

    cutClubs(&clubs_2, &clubs_count_2, 2);
...
int copyClubs(sClub_ptr clubs_2, int clubs_count_2, sClub_ptr clubs_1, int clubs_count_1, int clubs_count)
{
    int clubs_count_copied = 0;

    if( clubs_count )
    {
        if(clubs_count_1 < clubs_count)
            clubs_count = clubs_count_1;
    }
    else
        clubs_count = clubs_count_1;

    if(clubs_count_2 < clubs_count)
        clubs_count = clubs_count_2;

    while(clubs_count--)
    {
        *(clubs_2 + clubs_count) = *(clubs_1 + clubs_count);
        clubs_count_copied++;
    }

    return clubs_count_copied;
}

void sortClubs(sClub_ptr clubs,  int clubs_count)
{
   int i = 0;
   int j = 0;
   sClub tmp_club;
...
void cutClubs(sClub_ptr * clubs, int * clubs_count, int del_count)
{
    size_t sClub_size = sizeof(sClub);

    if( del_count >= *clubs_count )
    {
        freeClubs(clubs, clubs_count);
        return;
    }

    *clubs_count -= del_count;
