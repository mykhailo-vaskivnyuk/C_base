...
static int   tre_scan_nodes  (sNode *, int, handle_node);
//----------------------------------------------------------
...
{  /*-----------------------------------------------
   ...
   -----------------------------------------------*/
    ...
    return tre_scan_nodes(&tree->tree_root, 0, hdl_node);
}

int tre_scan_nodes(sNode * node, int node_level, handle_node hdl_node)
{  /*-----------------------------------------------
   || 0: помилка;                                 ||
   || 1: знайшли;                                 ||
   ||-1: 1)успішно оброблено;                     ||
   ...
   -----------------------------------------------*/
    ...
    if( scan_type == 0 &&
       (handle_result = hdl_node(node, node_level)) >= 0 )
            return handle_result;

    if( (handle_result = tre_scan_nodes(&node_left, node_level + 1, hdl_node)) >= 0 )
        return handle_result;
    ...
    if( (handle_result = tre_scan_nodes(&node_right, node_level + 1, hdl_node)) >= 0 )
        return handle_result;

    if( scan_type == 2 )
       handle_result = hdl_node(node, node_level);
